...
closed = cv2.dilate(canny, None, iterations=4)
#cv2.imshow('canny-1', closed)

closed = cv2.erode(closed, None, iterations=4)
#cv2.imshow('canny-2', closed)

# 形态学：边缘检测
_,Thr_img = cv2.threshold(canny,210,255,cv2.THRESH_BINARY)#设定红色通道阈值210（阈值影响梯度运算效果）
cv2.imshow('canny1', Thr_img)
kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(25,10))         #定义矩形结构元素
